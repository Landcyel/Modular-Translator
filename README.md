...
- **分割**：`split(text)` 一次处理全文——外层按 `\n` 分行，内层对每行做 token 扫描，产出 `[prefix][body][suffix]` 结构段（纯正文段为 `[body]`，一行可含多个段；空行原样保留）。配置 `region` 时，跨行符号对（如 ERB 的 `{...}` 块）包裹的部分按**虚拟行**应用规则后**再分行**回物理行（LineInfo 恒为物理行）。
- **占位符保护**：`placeholder` 命中的标记在分割阶段即从正文 `body` 中替换为唯一标记——序号放入 `<<<>>>` 内部（如 `<<<PH#0>>>`）；跨行匹配（含 `\n`）按行片段各发一标记并保留 `\n`；翻译后由 `merge()` 逐一原样还原，防止模型改动不可变内容（仅作用于 `body`，不作用于 `prefix`/`suffix`）。
...
每个规则文件是一个 JSON，可选以下 7 个键（均为数组）：

| 键 | 含义 |
|---|---|
| `prefix` | 前缀结构条目列表：识别行内的起始结构标记（token 扫描按任意位置匹配，非仅行首） |
| `suffix` | 后缀结构条目列表：识别行内的结束结构标记（同，非仅行尾） |
| `placeholder` | 二维数组 `[[匹配条目, 替换文本], ...]`：保护不可变标记（跨行匹配按行片段发标记） |
| `skip` | 可选：命中的行**整体不翻译**、原样透传（优先级最高） |
| `recognize` | 可选：过滤器。未配置 = 所有行都翻译；配置后 = 只有匹配的行才翻译，其余整行透传 |
| `ps_pair` | 可选：成对 prefix-suffix 数组，每项 `{"open": …, "close": …}`；成对结构识别（成对优先） |
| `region` | 可选：跨行区域数组，每项 `{"label": {"open": "{", "close": "}"}, "concat": ["+"]}`；label 为跨行符号对（开/闭行不入内容），concat 为行尾续行符（补充：不独立开启区域，仅延续已开启区域） |

**条目格式**（`prefix`/`suffix`/`skip`/`recognize` 的元素，以及 `ps_pair` 的 open/close、`region` 的 label open/close 与 concat）：
...
**匹配语义**：`prefix` 与 `suffix` 在行内**独立识别、无需配对**；token 级扫描（正则锚定当前位置，**最长匹配优先**，同长时前缀优先），其余文本为正文 `body`；连续多个前缀聚合为一个结构段的 prefix 槽、连续后缀聚合为 suffix 槽——**结构段**聚合形态恒为 `[prefix][body][suffix]`，**纯正文段**为 `[body]`，一行可产出多个段。仅纯正文段按句末标点（`。！？!?`）拆句，括号/引号内的标点不参与；结构段不拆句。零宽匹配的规则（如 `{"regex": "$"}`）不产生 token，实际无效。**`ps_pair` 成对条目优先**：同一位置按 成对 close > 成对 open > 独立 prefix > 独立 suffix 判定，保证括号结构不被独立标记抢占；未配对的定界符由独立前缀/后缀条目兜底。
