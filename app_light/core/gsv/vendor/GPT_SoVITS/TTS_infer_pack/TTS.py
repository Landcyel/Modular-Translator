...
try:
    from app.ffmpeg import FFMPEG_BIN
except Exception:
    FFMPEG_BIN = "ffmpeg"
...
class TTS:
    ...
    def init_cnhuhbert_weights(self, base_path: str):
        _t0 = time.time()
        logger.info(f"[gsv] 加载 CNHuBERT 权重: {base_path}")
        self.cnhuhbert_model = CNHubert(base_path)
        self.cnhuhbert_model = self.cnhuhbert_model.eval()
        self.cnhuhbert_model = self.cnhuhbert_model.to(self.configs.device)
        if self.configs.is_half and str(self.configs.device) != "cpu":
            self.cnhuhbert_model = self.cnhuhbert_model.half()
        logger.info(f"[gsv] CNHuBERT 加载完成（{time.time() - _t0:.1f}s）")

    def init_bert_weights(self, base_path: str):
        _t0 = time.time()
        logger.info(f"[gsv] 加载 BERT 权重: {base_path}")
        ...
        logger.info(f"[gsv] BERT 加载完成（{time.time() - _t0:.1f}s）")

    def init_vits_weights(self, weights_path: str):
        _t0 = time.time()
        logger.info(f"[gsv] 加载 VITS 权重: {weights_path}")
        ...
        logger.info(f"[gsv] VITS 加载完成（{time.time() - _t0:.1f}s）")



    def init_t2s_weights(self, weights_path: str):
        _t0 = time.time()
        logger.info(f"[gsv] 加载 Text2Semantic 权重: {weights_path}")
        ...
        logger.info(f"[gsv] Text2Semantic 加载完成（{time.time() - _t0:.1f}s）")
    ...
    def init_sv_model(self):
        if self.sv_model is not None:
            return
        _t0 = time.time()
        logger.info(f"[gsv] 加载 SV 模型: {sv.sv_path}")
        self.sv_model = SV(self.configs.device, self.configs.is_half)
        logger.info(f"[gsv] SV 模型加载完成（{time.time() - _t0:.1f}s）")
