...
try:
    from app.ffmpeg import FFMPEG_BIN, FFPROBE_BIN
except Exception:
    FFMPEG_BIN = "ffmpeg"
    FFPROBE_BIN = "ffprobe"
...
def uvr(model_name, inp_root, save_root_vocal, paths, save_root_ins, agg, format0):
    ...
    try:
        ...
        for path in paths:
            ...
            try:
                info = ffmpeg.probe(inp_path, cmd=FFPROBE_BIN)
            ...
            if need_reformat == 1:
                ...
                os.system(f'"{FFMPEG_BIN}" -i "{inp_path}" -vn -acodec pcm_s16le -ac 2 -ar 44100 "{tmp_path}" -y')
                inp_path = tmp_path
            try:
                if done == 0:
                    pre_fun._path_audio_(inp_path, save_root_ins, save_root_vocal, format0, is_hp3)
